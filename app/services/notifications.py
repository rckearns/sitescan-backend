...
async def send_email_alert(to_email: str, subject: str, html_body: str) -> bool:
    ...
    if not settings.postmark_api_key:
        logger.warning("Postmark not configured (POSTMARK_API_KEY not set), skipping email alert")
        return False
...
async def process_alerts():
    ...
    2. Score >= the user's minimum match threshold (computed dynamically)
    ...
    async with session_factory() as session:
        try:
            ...
            # Fetch new projects from the last scan cycle
            ...
            for user in users:
                # Score dynamically against this user's criteria
                scored = [
                    (p, score_against_profile(p, user))
                    for p in recent_projects
                ]
                qualifying = [
                    p for p, score in scored
                    if score >= (user.min_match_score or 0)
                ]
                ...
                to_alert = [p for p in qualifying if p.id not in already_alerted]
